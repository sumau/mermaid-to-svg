...
# mermaid/generated/, using the pinned mermaid-cli Docker image. The only local
# dependency is Docker: no Node, npm, Puppeteer, or Chromium install required.
#
# The same script runs locally (commit the result) and in CI (which re-runs it
# and fails if the result isn't already committed), so there is one definition
# of how diagrams are produced.
#
# Usage: ./render.sh   (or: make diagrams)
